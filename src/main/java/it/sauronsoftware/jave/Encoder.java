...
public class Encoder {
	...
	public MultimediaInfo getInfo(String sourceAbsolutePath) throws InputFormatException,
			EncoderException {
		FFMPEGExecutor ffmpeg = locator.createExecutor();
		ffmpeg.addArgument("-i");
		ffmpeg.addArgument(sourceAbsolutePath);
		try {
			ffmpeg.execute();
		} catch (IOException e) {
			throw new EncoderException(e);
		...
		try {
			...
			reader = new RBufferedReader(new InputStreamReader(ffmpeg
					.getErrorStream()));
			return parseMultimediaInfo(sourceAbsolutePath, reader);
		} finally {
			ffmpeg.destroy();
		}
	}

	private MultimediaInfo parseMultimediaInfo(String sourceAbsolutePath,
		...
		try {
			...
			while (true) {
				...
				if (step == 0) {
					String token = sourceAbsolutePath + ": ";
